...
def train_epoch(loader, net, opt_net, epoch, file):
    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        if batch_idx % log_interval == 0:
            ...
            file.write('  Acc: {:.2f}'.format(acc))


def train(data, datadir, model, num_cls, file, outdir='',
    ...
    file.write('Training {} model for {}'.format(model, data))
    for epoch in range(num_epoch):
        train_epoch(train_data, net, opt_net, epoch, file)
    ...
    if test_data is not None:
        ...
        file.write('Evaluating {}-{} model on {} test set'.format(model, data, data))
        test(test_data, net)
